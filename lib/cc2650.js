...
var filename = 'audio'; // File to store audio data
var audioFlag = false; // Flag to stop saving audio data when there is no more audio from tag is expected
...
CC2650SensorTag.prototype.onAudioChange = function(data) {
  this.saveAudioData(data, function(audio) {
...
CC2650SensorTag.prototype.saveAudioData = function(data, callback) {
  ...
  if(sizeof(data) == 1) {
	audioCommand = data.readUInt8(0); 
	if(audioCommand == 0) { // 0x00 Received -> means the end of audio transmission
	  if(tagId == 'BLE_MAC_ADDRESS') {  // BLE MAC, e.g. 'a1e6f9af4c76'
		audioFlag = false; 
		callback(audioFlag);
	  } 
	} else if (audioCommand == 4) { // 0x04 Received -> means the start of audio transmission
	  // Do something here
      callback(true);
	} else {
	  // Should never get here -> add logging statement
	  callback(true);
	}
  } else { // Each piece of audio data is 20 bytes (1 frame = 100 bytes == 5 pieces)
	if(tagId == 'BLE_MAC_ADDRESS') {
	  fs.appendFile(filename, new Buffer(data), function (err) {
		if(err) throw err;
	  });
	  callback(audioFlag);
	}
