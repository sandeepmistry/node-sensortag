...
CC2650SensorTag.prototype.convertMPU9250Data = function(data, callback) {
  ...
  var x = data.readInt16LE(0) * 2.0 / 32768.0;
  var y = data.readInt16LE(2) * 2.0 / 32768.0;
  var z = data.readInt16LE(4) * 2.0 / 32768.0 + 1.0;

  // 250deg/s range
  var xG = data.readInt16LE(6) * (500.0 / 65536.0) * -1;
  var yG = data.readInt16LE(8) * (500.0 / 65536.0);
  var zG = data.readInt16LE(10) * (500.0 / 65536.0);
...
CC2650SensorTag.prototype.enableGyroscopeAxis = function(enableXAxis, enableYAxis, enableZAxis, callback) {
  this.enableMPU9250(MPU9250_GYROSCOPE_MASK, callback); // TODO: using axis
};
...
CC2650SensorTag.prototype.setLuxometerPeriod = function(period, callback) {
  this.writePeriodCharacteristic(LUXOMTER_UUID, LIGHT_PERIOD_UUID, period, callback);
};
