...
CC2650SensorTag.prototype.convertBarometricPressureData = function(data, callback) {
  ...
  // Martin Doyle
  // data is actually returned as two 24 bit values
  // Temp not fixed since there is no issue
  var exponent;
  var mantissa;
  ...
  // Martin Doyle
  // TI uses a function twentyFourBitUnsignedAtOffset
  // Values with the TI reading on my Android device are stable but still wrong
  // Changed the offset
  offset = 3; // Start at Byte 3 instead of 2
  lowerByte = data.readUInt8(offset) & 0xFF;
  mediumByte = data.readUInt8(offset+1) & 0xFF;
  upperByte = data.readUInt8(offset + 2) & 0xFF;
  // Martin Doyle
  // Now the division by 100.0 works
  flPressure = ((upperByte << 16) + (mediumByte << 8) + lowerByte) / 100.0;
